...
// InFlightDiagnostic represents a diagnostic message, including its template,
// level, message, and associated notes. An actual InFlightDiagnostic is
...
// DiagnosticEngine can configure exactly how they want the engine to process an
// InFlightDiagnostic.
...
func (s *SliceConsumer) Pop() string {
	// TODO : might be more efficient to impl with a list, low priority as this is for tests only
...
// NewDiagnosticEngine creates and returns a new instance of DiagnosticEngine.
// Initializes diagnostic settings and overrides as empty.
...
// Enable explicitly enables a diagnostic by its name, overriding default
// settings.
func (engine *DiagnosticEngine) Enable(diagName string) {
	diagTempl := diagnosticTemplate(diagName)
...
// Disable explicitly disables a diagnostic by its name, overriding default
// settings.
...
func (engine *DiagnosticEngine) Report(diagName string, messageParams map[string]string) {
	...
	// Issue separate diagnostics for any associated notes, we don't need to
	// track counts since notes are associated with a parent diagnostic, whose
	// count is being tracked already
