 ...
 * tile indices back to PackableTile::Id values. Strategies use this metadata to detect when a candidate palette already
 * contains a sibling tile from the same shape group, enabling sharing-aware cost penalties.
 *
 * This struct is intended to be stored as an optional field in PackingInput. When absent, strategies behave identically
 * to their non-sharing-aware versions.
 */
