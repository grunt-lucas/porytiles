...
std::unique_ptr<DecompiledTileset> decompile(PorytilesContext &ctx, DecompilerMode mode,
    ...
    for (std::size_t entryIndex = 0; const auto &metatileEntry : compiledTileset.metatileEntries) {
        ...
        // secondary
        if (mode == DecompilerMode::SECONDARY) {
            // TODO : there are four cases we need to handle here, we are currently only handling two
            //
            ...
            // This case is handled below, though we aren't doing an explicit check for the pal index range.
            // TODO : we should add the explicit check mentioned above
            //
            ...
            // re-colored tree example in the Palette Overrides wiki page.
            // TODO : we NEED to handle case 2, this is the bug reported in Issue #111
            //
            ...
            // TODO : we NEED to handle case 3, the reason we didn't have a bug reported here is mostly because this
            // isn't super common, and because often the secondary palettes in primary range (if the tileset is from
            // vanilla) have been correctly set with the primary colors. This is common but not guaranteed so we
            // shouldn't assume it's the case
            //
            ...
            // super common of course. It is implicitly handled by our "else" block below.
            // TODO : we should handle this explicitly
             ...
             * the foliage colors from the primary set.
             *
             * The reason we didn't have a bug reported here is mostly because this
             * isn't super common, and because often the secondary palettes in primary range (if the tileset is from
             * vanilla) have been correctly set with the primary colors. This is common but not guaranteed so we
             * shouldn't assume it's the case
             */
