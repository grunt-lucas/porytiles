...
DecompiledTileset importTilesFromPng(PorytilesContext &ctx, CompilerMode compilerMode,
    ...
    if (png.get_height() % TILE_SIDE_LENGTH_PIX != 0) {
        ctx.diag->Report(kErrGeneric, fmt::format("source tiles PNG height '{}' was not divisible by 8",
                                                  ctx.diag->Bold(png.get_height())));
    }
    if (png.get_width() % TILE_SIDE_LENGTH_PIX != 0) {
        ctx.diag->Report(kErrGeneric, fmt::format("source tiles PNG width '{}' was not divisible by 8",
...
DecompiledTileset importLayeredTilesFromPngs(PorytilesContext &ctx, CompilerMode compilerMode,
    ...
    if (bottom.get_height() % METATILE_SIDE_LENGTH != 0) {
        ctx.diag->Report(kErrGeneric, fmt::format("{} layer source PNG height '{}' was not divisible by 16",
    ...
    for (const auto &metatileId : attributesMap | std::views::keys) {
        if (metatileId > metatileCount - 1) {
            ctx.diag->Report(kWarnUnusedAttribute, ctx.diag->Bold(metatileId));
            ctx.diag->ReportPartner(kWarnUnusedAttribute, 0, metatileCount,
...
static void validateAnimFormat(const PorytilesContext &ctx, const DecompiledAnimation &anim) {
    ...
    for (std::size_t i = 0; i < keyFrameMissingColors.size(); i++) {
        if (!keyFrameMissingColors.at(i).empty()) {
            ctx.diag->Report(kWarnKeyFrameMissingColors, anim.animName, i);
            std::vector<RGBA32> v{keyFrameMissingColors.at(i).begin(), keyFrameMissingColors.at(i).end()};
            ctx.diag->ReportPartner(kWarnKeyFrameMissingColors, 0, std::move(v));
        }
    }
}
...
void importAnimTiles(PorytilesContext &ctx, CompilerMode compilerMode,
    ...
    for (const auto &rawAnim : rawAnims) {
        ...
        for (const auto &rawFrame : rawAnim) {
            ...
            if (rawFrame.png.get_height() % TILE_SIDE_LENGTH_PIX != 0) {
                ctx.diag->Report(kErrGeneric, fmt::format("anim {} frame {} PNG height '{}' was not divisible by 8",
            ...
            if (rawFrame.png.get_width() % TILE_SIDE_LENGTH_PIX != 0) {
                ctx.diag->Report(kErrGeneric, fmt::format("anim {} frame {} PNG width '{}' was not divisible by 8",
                                                          rawFrame.animName, rawFrame.frameName,
    ...
    if (ctx.diag->EnabledAt(kWarnKeyFrameMissingColors) == DiagLevel::Error &&
        ctx.diag->InFlightCountFor(kWarnKeyFrameMissingColors) > 0) {
        die_errorCount(ctx, ctx.compilerSrcPaths.modeBasedSrcPath(compilerMode),
...
importAttributesFromCsv(PorytilesContext &ctx, CompilerMode compilerMode,
    ...
    try {
        ...
        ctx.diag->Report(kFatalGeneric, msg);
        ctx.diag->Report(kNoteGeneric, fmt::format("valid headers are '{}' or '{}'", ctx.diag->Bold("id,behavior"),
    ...
    if (ctx.targetBaseGame == TargetBaseGame::FIRERED && (!hasTerrainType || !hasEncounterType)) {
        ctx.diag->Report(kWarnAttributeFormatMismatch, ctx.diag->Bold(filePath), "few",
                         ctx.diag->Bold(targetBaseGameString(ctx.targetBaseGame)));
        ctx.diag->ReportPartner(kWarnAttributeFormatMismatch, 0);
    }
    if ((ctx.targetBaseGame == TargetBaseGame::EMERALD || ctx.targetBaseGame == TargetBaseGame::RUBY) &&
        (hasTerrainType || hasEncounterType)) {
        ctx.diag->Report(kWarnAttributeFormatMismatch, ctx.diag->Bold(filePath), "many",
                         ctx.diag->Bold(targetBaseGameString(ctx.targetBaseGame)));
    }
...
RGBATile importPalettePrimer(PorytilesContext &ctx, const CompilerMode compilerMode, std::ifstream &paletteFile,
    ...
    if (declaredPaletteSize == 0 || declaredPaletteSize > PAL_SIZE - 1) {
        ctx.diag->Report(kErrGeneric, fmt::format("{} invalid declared size '{}', must be 1 <= size <= 15",
    ...
    while (std::getline(paletteFile, line)) {
        ...
        if (rgbaToBgr(rgba) == rgbaToBgr(ctx.compilerConfig.transparencyColor)) {
            ctx.diag->Report(kErrGeneric, fmt::format("{} '{}' was transparent or collapsed to transparent",
                                                      ctx.diag->Bold(fileName + ":"), ctx.diag->Bold(rgba.jasc())));
        }
    ...
    if (usedPaletteCount != declaredPaletteSize) {
        ctx.diag->Report(kErrGeneric, fmt::format("{} used pal size ({}) did not match declared size '{}'",
...
std::pair<RGBATile, OverridenPaletteSlots> importPaletteOverride(PorytilesContext &ctx, const CompilerMode compilerMode,
    ...
    if (usedPaletteCount != declaredPaletteSize) {
        ctx.diag->Report(kErrGeneric, fmt::format("{} used palette size ({}) did not match declared size '{}'",
                                                  ctx.diag->Bold(fileName + ":"), usedPaletteCount,
                                                  ctx.diag->Bold(declaredPaletteSize)));
    }
...
TEST_CASE("importAnimTiles should read each animation and correctly populate the DecompiledTileset anims field") {
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_white/expected/frame0_tile0.png"}));
    png::image<png::rgba_pixel> frame0Tile0Png{"Resources/Tests/anim_flower_white/expected/frame0_tile0.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_white/expected/frame0_tile1.png"}));
    png::image<png::rgba_pixel> frame0Tile1Png{"Resources/Tests/anim_flower_white/expected/frame0_tile1.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_white/expected/frame0_tile2.png"}));
    png::image<png::rgba_pixel> frame0Tile2Png{"Resources/Tests/anim_flower_white/expected/frame0_tile2.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_white/expected/frame0_tile3.png"}));
    png::image<png::rgba_pixel> frame0Tile3Png{"Resources/Tests/anim_flower_white/expected/frame0_tile3.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_white/expected/frame1_tile0.png"}));
    png::image<png::rgba_pixel> frame1Tile0Png{"Resources/Tests/anim_flower_white/expected/frame1_tile0.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_white/expected/frame1_tile1.png"}));
    png::image<png::rgba_pixel> frame1Tile1Png{"Resources/Tests/anim_flower_white/expected/frame1_tile1.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_white/expected/frame1_tile2.png"}));
    png::image<png::rgba_pixel> frame1Tile2Png{"Resources/Tests/anim_flower_white/expected/frame1_tile2.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_white/expected/frame1_tile3.png"}));
    png::image<png::rgba_pixel> frame1Tile3Png{"Resources/Tests/anim_flower_white/expected/frame1_tile3.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_white/expected/frame2_tile0.png"}));
    png::image<png::rgba_pixel> frame2Tile0Png{"Resources/Tests/anim_flower_white/expected/frame2_tile0.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_white/expected/frame2_tile1.png"}));
    png::image<png::rgba_pixel> frame2Tile1Png{"Resources/Tests/anim_flower_white/expected/frame2_tile1.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_white/expected/frame2_tile2.png"}));
    png::image<png::rgba_pixel> frame2Tile2Png{"Resources/Tests/anim_flower_white/expected/frame2_tile2.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_white/expected/frame2_tile3.png"}));
    png::image<png::rgba_pixel> frame2Tile3Png{"Resources/Tests/anim_flower_white/expected/frame2_tile3.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_yellow/expected/frame0_tile0.png"}));
    png::image<png::rgba_pixel> frame0Tile0Png_yellow{"Resources/Tests/anim_flower_yellow/expected/frame0_tile0.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_yellow/expected/frame0_tile1.png"}));
    png::image<png::rgba_pixel> frame0Tile1Png_yellow{"Resources/Tests/anim_flower_yellow/expected/frame0_tile1.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_yellow/expected/frame0_tile2.png"}));
    png::image<png::rgba_pixel> frame0Tile2Png_yellow{"Resources/Tests/anim_flower_yellow/expected/frame0_tile2.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_yellow/expected/frame0_tile3.png"}));
    png::image<png::rgba_pixel> frame0Tile3Png_yellow{"Resources/Tests/anim_flower_yellow/expected/frame0_tile3.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_yellow/expected/frame1_tile0.png"}));
    png::image<png::rgba_pixel> frame1Tile0Png_yellow{"Resources/Tests/anim_flower_yellow/expected/frame1_tile0.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_yellow/expected/frame1_tile1.png"}));
    png::image<png::rgba_pixel> frame1Tile1Png_yellow{"Resources/Tests/anim_flower_yellow/expected/frame1_tile1.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_yellow/expected/frame1_tile2.png"}));
    png::image<png::rgba_pixel> frame1Tile2Png_yellow{"Resources/Tests/anim_flower_yellow/expected/frame1_tile2.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_yellow/expected/frame1_tile3.png"}));
    png::image<png::rgba_pixel> frame1Tile3Png_yellow{"Resources/Tests/anim_flower_yellow/expected/frame1_tile3.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_yellow/expected/frame2_tile0.png"}));
    png::image<png::rgba_pixel> frame2Tile0Png_yellow{"Resources/Tests/anim_flower_yellow/expected/frame2_tile0.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_yellow/expected/frame2_tile1.png"}));
    png::image<png::rgba_pixel> frame2Tile1Png_yellow{"Resources/Tests/anim_flower_yellow/expected/frame2_tile1.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_yellow/expected/frame2_tile2.png"}));
    png::image<png::rgba_pixel> frame2Tile2Png_yellow{"Resources/Tests/anim_flower_yellow/expected/frame2_tile2.png"};
    ...
    REQUIRE(
        std::filesystem::exists(std::filesystem::path{"Resources/Tests/anim_flower_yellow/expected/frame2_tile3.png"}));
    png::image<png::rgba_pixel> frame2Tile3Png_yellow{"Resources/Tests/anim_flower_yellow/expected/frame2_tile3.png"};
...
TEST_CASE("importLayeredTilesFromPngs should correctly import a dual layer tileset via layer type inference") {
    ...
    REQUIRE(std::filesystem::exists(std::filesystem::path{"Resources/Tests/dual_layer_metatiles_1/bottom.png"}));
    REQUIRE(std::filesystem::exists(std::filesystem::path{"Resources/Tests/dual_layer_metatiles_1/middle.png"}));
    REQUIRE(std::filesystem::exists(std::filesystem::path{"Resources/Tests/dual_layer_metatiles_1/top.png"}));

    png::image<png::rgba_pixel> bottom{"Resources/Tests/dual_layer_metatiles_1/bottom.png"};
    png::image<png::rgba_pixel> middle{"Resources/Tests/dual_layer_metatiles_1/middle.png"};
    png::image<png::rgba_pixel> top{"Resources/Tests/dual_layer_metatiles_1/top.png"};
...
TEST_CASE("importCompiledTileset should import a triple-layer pokeemerald tileset correctly") {
    ...
    compileCtx.compilerSrcPaths.primarySourcePath = "Resources/Tests/anim_metatiles_2/primary";
    REQUIRE(std::filesystem::exists(std::filesystem::path{"Resources/Tests/metatile_behaviors.h"}));
    compileCtx.compilerSrcPaths.metatileBehaviors = "Resources/Tests/metatile_behaviors.h";
