...
- Added a fully configurable metatile attribute schema, replacing the hardcoded `BaseGame` (Emerald vs. FireRed/LeafGreen) system. A project has exactly one attribute schema, inferred from its own headers by default or declared explicitly via `metatile_attribute_fields`. The attribute entry size is inferred from the project's attribute masks. `pokeemerald-expansion` declares both the emerald and FRLG mask layouts (one ROM only ever links one, chosen by `make` vs `make firered`), so there `metatile_attribute_size` must be set explicitly, one value per build type, and it selects the matching mask layout. A new `metatile_attribute_declaration_size` config controls the declared element width of generated `gMetatileAttributes_*` arrays (inferred from `struct Tileset`), covering expansion's FRLG build where 4-byte entries are declared `const u16`. - [#341](https://github.com/grunt-lucas/porytiles/pull/341) [#346](https://github.com/grunt-lucas/porytiles/pull/346)
...
- Changed metatile attribute schema resolution to reconcile the project's inferred mask layout with the user's config in one place, instead of deriving values inside the config provider chain. `metatile_attribute_size` is now a true override knob (unset means "derive from the project's masks") rather than a defaulted setting, and `dump-tileset-config` reports where the resolved attribute size and declaration size came from. Scan and inference warnings (tag `metatile-attr-inference`) now respect the `--diagnostic-warnings-include`/`-exclude` filters; they previously bypassed them. ([#336](https://github.com/grunt-lucas/porytiles/issues/336))
