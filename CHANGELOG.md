...
- Added a fully configurable metatile attribute schema, replacing the hardcoded `BaseGame` (Emerald vs. FireRed/LeafGreen) system. Attribute layout, bit masks, and per-field enum sources are now handled by the config system, inferred from the project's own headers by default or declared explicitly via `metatile_attribute_fields`, with per-tileset overrides for FRLG's alternate mask layout and layer type - [#341](https://github.com/grunt-lucas/porytiles/pull/341)
