...
enum Subcommand { COMPILE_RAW, COMPILE };
...
struct Config {
  ...
  void validate() const
  {
    if (numTilesInPrimary > numTilesTotal) {
      throw PtException{"fieldmap parameter `numTilesInPrimary' (" + std::to_string(numTilesInPrimary) +
                        ") exceeded `numTilesTotal' (" + std::to_string(numTilesTotal) + ")"};
    }
    if (numMetatilesInPrimary > numMetatilesTotal) {
      throw PtException{"fieldmap parameter `numMetatilesInPrimary' (" + std::to_string(numMetatilesInPrimary) +
                        ") exceeded `numMetatilesTotal' (" + std::to_string(numMetatilesTotal) + ")"};
    }
    if (numPalettesInPrimary > numPalettesTotal) {
      throw PtException{"fieldmap parameter `numPalettesInPrimary' (" + std::to_string(numPalettesInPrimary) +
