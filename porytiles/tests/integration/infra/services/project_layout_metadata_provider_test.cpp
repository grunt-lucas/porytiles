...
// Fixture with a hand-built layouts.json exercising every layout_version_usage bucket.
class LayoutVersionUsageTest : public ProjectLayoutMetadataProviderTestBase {
  protected:
    [[nodiscard]] std::filesystem::path project_root_path() const override
    {
        return "resources/tests/integration/infra/services/layout_version_usage";
    }
};

TEST_F(LayoutVersionUsageTest, FrlgOnlyTilesetClassifiesFrlg)
{
    auto result = layout_provider_->layout_version_usage("gTileset_FrlgOnly");
    ASSERT_TRUE(result.has_value());
    EXPECT_EQ(result.value(), TilesetLayoutVersionUsage::frlg_only);
}

TEST_F(LayoutVersionUsageTest, EmeraldOnlyTilesetClassifiesEmerald)
{
    auto result = layout_provider_->layout_version_usage("gTileset_EmeraldOnly");
    ASSERT_TRUE(result.has_value());
    EXPECT_EQ(result.value(), TilesetLayoutVersionUsage::emerald_only);
}

TEST_F(LayoutVersionUsageTest, AbsentLayoutVersionCountsAsEmerald)
{
    auto result = layout_provider_->layout_version_usage("gTileset_NoVersion");
    ASSERT_TRUE(result.has_value());
    EXPECT_EQ(result.value(), TilesetLayoutVersionUsage::emerald_only);
}

TEST_F(LayoutVersionUsageTest, MixedUsageClassifiesMixed)
{
    auto result = layout_provider_->layout_version_usage("gTileset_Mixed");
    ASSERT_TRUE(result.has_value());
    EXPECT_EQ(result.value(), TilesetLayoutVersionUsage::mixed);
}

TEST_F(LayoutVersionUsageTest, UnreferencedTilesetClassifiesUnreferenced)
{
    auto result = layout_provider_->layout_version_usage("gTileset_NotInAnyLayout");
    ASSERT_TRUE(result.has_value());
    EXPECT_EQ(result.value(), TilesetLayoutVersionUsage::unreferenced);
}

TEST_F(LayoutVersionUsageTest, InvalidLayoutVersionOnReferencingLayoutIsHardError)
{
    auto result = layout_provider_->layout_version_usage("gTileset_BadVersion");
    ...
    // The message must name the layout, the bad value, and the two valid values.
    EXPECT_NE(text.find("BadVersion_Layout"), std::string::npos) << text;
    EXPECT_NE(text.find("firered"), std::string::npos) << text;
    EXPECT_NE(text.find("emerald"), std::string::npos) << text;
    EXPECT_NE(text.find("frlg"), std::string::npos) << text;
}

// A non-string layout_version (number, null, object) on a referencing layout must be a hard error too, not silently
// read as Emerald. schema width must not ride on a typo, and JSON type confusion is a typo.
TEST_F(LayoutVersionUsageTest, NonStringLayoutVersionOnReferencingLayoutIsHardError)
{
    for (const auto &tileset : {"gTileset_NumberVersion", "gTileset_NullVersion", "gTileset_ObjectVersion"}) {
        auto result = layout_provider_->layout_version_usage(tileset);
        ASSERT_FALSE(result.has_value()) << "expected hard error for " << tileset;
        ...
        // Still an "invalid layout_version value" error naming the valid values, same as a bad string.
        EXPECT_NE(text.find("emerald"), std::string::npos) << text;
        EXPECT_NE(text.find("frlg"), std::string::npos) << text;
    }
}

// Fixture pointing at a project with no layouts.json at all.
class LayoutVersionUsageNoFileTest : public ProjectLayoutMetadataProviderTestBase {
  protected:
    [[nodiscard]] std::filesystem::path project_root_path() const override
    {
        return "resources/tests/integration/infra/services/layout_version_usage_no_layouts";
    }
};

TEST_F(LayoutVersionUsageNoFileTest, MissingLayoutsFileYieldsUnreferenced)
{
    auto result = layout_provider_->layout_version_usage("gTileset_Anything");
    ASSERT_TRUE(result.has_value());
    EXPECT_EQ(result.value(), TilesetLayoutVersionUsage::unreferenced);
}
