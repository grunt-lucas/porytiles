...
- **A2a — Directory + CMake skeleton**:
  ...
  - In `docs/CMakeLists.txt`: update `Porytiles2Lib`, `PORYTILES2_PUBLIC_HEADER*`, and the three `${PROJECT_SOURCE_DIR}/Porytiles2/{include,lib,tools}` paths.
...
- Verify the rest of the workflow (build target paths, deploy step) continues to work after the Phase A rename — paths like `./build/docs/doxygen/html` need to be confirmed against the post-rename CMake layout.
...
**Documentation build**:
- `/Users/lucas/Projects/github.com/grunt-lucas/porytiles/docs/CMakeLists.txt`
