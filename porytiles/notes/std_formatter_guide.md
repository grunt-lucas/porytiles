# std::formatter Guide for Porytiles
...
used an ADL hack with `using std::to_string; using porytiles::to_string;` which was fragile
and hard to understand.

### The Relationship

The fix enables a clean pipeline:
1. Custom types provide `porytiles::to_string()` overloads
...
```c++
// 1. In the type's header, inside namespace porytiles:
inline std::string to_string(const MyType &val) { /* ... */ }

// 2. After closing namespace porytiles:
template <>
struct std::formatter<porytiles::MyType> {
    ...
    auto format(const porytiles::MyType &value, auto &ctx) const  // MUST be auto &ctx
    {
