...
Quick-reference recipes for the Porytiles diagnostic and formatting APIs, drawn from real
codebase usage. Each section covers one API surface with copy-pasteable patterns.

**Key headers:**

```c++
#include "porytiles/utilities/text/text_formatter.hpp"       // Style, FormatParam, TextFormatter
#include "porytiles/utilities/result/error.hpp"              // Error, FormattableError
#include "porytiles/utilities/result/chainable_result.hpp"   // ChainableResult, PT_TRY_* macros
#include "porytiles/xcut/diagnostics/user_diagnostics.hpp"   // UserDiagnostics
#include "porytiles/domain/algorithms/diagnostic_stencils.hpp" // Stencil helpers
```
...
Custom types need a `std::formatter<T>` specialization that delegates to `porytiles::to_string()`.
