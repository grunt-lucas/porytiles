...
/// Implementations handle the specifics of how to organize and access artifacts in different backing stores
...
/// particular implementation requires a complex key type, the implementation must take care to ensure that this key
/// type is string-constructable. Luckily, rich string-based formats like JSON should be able to support almost any
/// imaginable key schema.
class TilesetArtifactKeyProvider {
  public:
    ...
    /// Porymap animation. The returned indices should be consecutive starting from 1. Since frame 0 is required,
    ...
    ///
    /// @details
    /// Each Porymap artifact has a unique key by which the ArtifactChecksumProvider and the TilesetRepo can identify
