...
class TilesetArtifactWriter {
  public:
    ...
    [[nodiscard]] virtual ChainableResult<void>
    write_porymap_palette_n(const ArtifactKey &dest_key, const Tileset &src, std::size_t index) = 0;
    ...
    [[nodiscard]] virtual ChainableResult<void>
    write_porytiles_palette_n(const ArtifactKey &dest_key, const Tileset &src, std::size_t index) = 0;
    ...
    ///
    /// @details
    /// Generates and writes the animation parameters (frame sequences, timing, etc.) for all animations in the
    /// Porytiles component.
    ///
    /// @param dest_key Key identifying where to write the parameters
