...
///
/// @details
/// IndirectPosition says "my slot is wherever @c ref_color ends up in palette @c ref_palette_index." This is the key
/// insight from borytiles by ishax-kos (https://github.com/ishax-kos/borytiles): by referencing colors rather than
/// absolute slots, links remain valid even when sequential fill places non-shared colors at different positions than
/// the base palettes. Corresponds to the @c Indirect(Palette_index,Color_index) variant of borytiles'
/// @c Position_in_palette enum.
///
/// Indirect chains are resolved after sequential fill by following @c ref_palette_index / @c ref_color until hitting an
/// AbsolutePosition. Cycle detection caps resolution at @c palette::num_palettes iterations.
...
///
/// @details
/// An IndirectLink says: "source_color in palette source_palette should occupy the same slot as ref_color in palette
/// ref_palette." During palette construction, this translates to setting source_color's position to
/// IndirectPosition{ref_palette, ref_color}. When resolved, both colors end up at the same slot index in their
/// respective palettes, enabling tile sharing.
///
/// @invariant source_palette != ref_palette (links always cross palette boundaries)
struct IndirectLink {
    /// @brief The palette index containing the color to be linked.
    std::size_t source_palette;

    /// @brief The color in source_palette that should follow the reference.
    Rgba32 source_color;

    /// @brief The palette index containing the reference color.
    std::size_t ref_palette;

    /// @brief The color in ref_palette whose slot position should be followed.
    Rgba32 ref_color;
