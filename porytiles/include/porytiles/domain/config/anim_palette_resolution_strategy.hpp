#pragma once

#include <algorithm>
#include <cctype>
#include <format>
#include <optional>
#include <ostream>
#include <string>

#include "porytiles/utilities/panic/panic.hpp"

// NOTE: DO NOT EDIT THIS FILE DIRECTLY. It is AUTO-GENERATED from config_schema.yaml.
// To add new config values or make other changes, edit config_schema.yaml and regenerate via:
//
//   uv run scripts/generate_config.py

namespace porytiles {

/// @brief Strategy for determining which palette to use when decompiling animation tiles.
///
/// @details
/// When decompiling animation tiles, the decompiler needs to know which tileset palette to use for
/// converting indexed pixels to RGBA. This enum determines the strategy used to find that palette.
/// The strategy is applied directly.
///
enum class AnimPaletteResolutionStrategy {
    /// @brief Scan local tileset metatile entries to find palette references for the animation tiles.
    scan_local_metatiles,
    /// @brief Use palette 00 directly.
    palette_00,
    /// @brief Use palette 01 directly.
    palette_01,
    /// @brief Use palette 02 directly.
    palette_02,
    /// @brief Use palette 03 directly.
    palette_03,
    /// @brief Use palette 04 directly.
    palette_04,
    /// @brief Use palette 05 directly.
    palette_05,
    /// @brief Use palette 06 directly.
    palette_06,
    /// @brief Use palette 07 directly.
    palette_07,
    /// @brief Use palette 08 directly.
    palette_08,
    /// @brief Use palette 09 directly.
    palette_09,
    /// @brief Use palette 10 directly.
    palette_10,
    /// @brief Use palette 11 directly.
    palette_11,
    /// @brief Use palette 12 directly.
    palette_12,
    /// @brief Use palette 13 directly.
    palette_13,
    /// @brief Use palette 14 directly.
    palette_14,
    /// @brief Use palette 15 directly.
    palette_15,
    /// @brief Use the Porymap-component frame PNG internal palette, if present.
    internal_png_palette,
    /// @brief Scan layouts config and find all tilesets paired with current tileset, see if an index is present in any.
    scan_all_tilesets
};

/// @brief Parses a string into a AnimPaletteResolutionStrategy with fuzzy matching.
///
/// @details
/// Performs parsing in two phases:
/// 1. Exact match against C++ constant names (snake_case)
/// 2. Case-insensitive fuzzy match against configured fuzzy_names
///
/// This unified function is used by both CLI and YAML parsing. The fuzzy matching
/// allows users to use various common formats (kebab-case, snake_case, etc.) while
/// still getting good error messages through LazyLayeredConfig.
///
/// @param str The string to parse
/// @return The parsed value, or std::nullopt if the string is invalid
[[nodiscard]] inline std::optional<AnimPaletteResolutionStrategy>
anim_palette_resolution_strategy_from_str(const std::string &str)
{
    // Phase 1: Exact match against C++ constant names
    if (str == "scan_local_metatiles") {
        return std::optional{AnimPaletteResolutionStrategy::scan_local_metatiles};
    }
    if (str == "palette_00") {
        return std::optional{AnimPaletteResolutionStrategy::palette_00};
    }
    if (str == "palette_01") {
        return std::optional{AnimPaletteResolutionStrategy::palette_01};
    }
    if (str == "palette_02") {
        return std::optional{AnimPaletteResolutionStrategy::palette_02};
    }
    if (str == "palette_03") {
        return std::optional{AnimPaletteResolutionStrategy::palette_03};
    }
    if (str == "palette_04") {
        return std::optional{AnimPaletteResolutionStrategy::palette_04};
    }
    if (str == "palette_05") {
        return std::optional{AnimPaletteResolutionStrategy::palette_05};
    }
    if (str == "palette_06") {
        return std::optional{AnimPaletteResolutionStrategy::palette_06};
    }
    if (str == "palette_07") {
        return std::optional{AnimPaletteResolutionStrategy::palette_07};
    }
    if (str == "palette_08") {
        return std::optional{AnimPaletteResolutionStrategy::palette_08};
    }
    if (str == "palette_09") {
        return std::optional{AnimPaletteResolutionStrategy::palette_09};
    }
    if (str == "palette_10") {
        return std::optional{AnimPaletteResolutionStrategy::palette_10};
    }
    if (str == "palette_11") {
        return std::optional{AnimPaletteResolutionStrategy::palette_11};
    }
    if (str == "palette_12") {
        return std::optional{AnimPaletteResolutionStrategy::palette_12};
    }
    if (str == "palette_13") {
        return std::optional{AnimPaletteResolutionStrategy::palette_13};
    }
    if (str == "palette_14") {
        return std::optional{AnimPaletteResolutionStrategy::palette_14};
    }
    if (str == "palette_15") {
        return std::optional{AnimPaletteResolutionStrategy::palette_15};
    }
    if (str == "internal_png_palette") {
        return std::optional{AnimPaletteResolutionStrategy::internal_png_palette};
    }
    if (str == "scan_all_tilesets") {
        return std::optional{AnimPaletteResolutionStrategy::scan_all_tilesets};
    }

    // Phase 2: Case-insensitive fuzzy match
    std::string lower_str = str;
    std::ranges::transform(
        lower_str, lower_str.begin(), [](unsigned char c) { return static_cast<char>(std::tolower(c)); });

    // Fuzzy names for scan_local_metatiles
    if (lower_str == "scan-local-metatiles") {
        return std::optional{AnimPaletteResolutionStrategy::scan_local_metatiles};
    }
    if (lower_str == "scan_local_metatiles") {
        return std::optional{AnimPaletteResolutionStrategy::scan_local_metatiles};
    }
    if (lower_str == "scanlocalmetatiles") {
        return std::optional{AnimPaletteResolutionStrategy::scan_local_metatiles};
    }
    // Fuzzy names for palette_00
    if (lower_str == "palette-00") {
        return std::optional{AnimPaletteResolutionStrategy::palette_00};
    }
    if (lower_str == "palette_00") {
        return std::optional{AnimPaletteResolutionStrategy::palette_00};
    }
    if (lower_str == "palette00") {
        return std::optional{AnimPaletteResolutionStrategy::palette_00};
    }
    if (lower_str == "pal-0") {
        return std::optional{AnimPaletteResolutionStrategy::palette_00};
    }
    if (lower_str == "pal_0") {
        return std::optional{AnimPaletteResolutionStrategy::palette_00};
    }
    if (lower_str == "pal0") {
        return std::optional{AnimPaletteResolutionStrategy::palette_00};
    }
    if (lower_str == "palette-0") {
        return std::optional{AnimPaletteResolutionStrategy::palette_00};
    }
    if (lower_str == "palette_0") {
        return std::optional{AnimPaletteResolutionStrategy::palette_00};
    }
    if (lower_str == "palette0") {
        return std::optional{AnimPaletteResolutionStrategy::palette_00};
    }
    // Fuzzy names for palette_01
    if (lower_str == "palette-01") {
        return std::optional{AnimPaletteResolutionStrategy::palette_01};
    }
    if (lower_str == "palette_01") {
        return std::optional{AnimPaletteResolutionStrategy::palette_01};
    }
    if (lower_str == "palette01") {
        return std::optional{AnimPaletteResolutionStrategy::palette_01};
    }
    if (lower_str == "pal-1") {
        return std::optional{AnimPaletteResolutionStrategy::palette_01};
    }
    if (lower_str == "pal_1") {
        return std::optional{AnimPaletteResolutionStrategy::palette_01};
    }
    if (lower_str == "pal1") {
        return std::optional{AnimPaletteResolutionStrategy::palette_01};
    }
    if (lower_str == "palette-1") {
        return std::optional{AnimPaletteResolutionStrategy::palette_01};
    }
    if (lower_str == "palette_1") {
        return std::optional{AnimPaletteResolutionStrategy::palette_01};
    }
    if (lower_str == "palette1") {
        return std::optional{AnimPaletteResolutionStrategy::palette_01};
    }
    // Fuzzy names for palette_02
    if (lower_str == "palette-02") {
        return std::optional{AnimPaletteResolutionStrategy::palette_02};
    }
    if (lower_str == "palette_02") {
        return std::optional{AnimPaletteResolutionStrategy::palette_02};
    }
    if (lower_str == "palette02") {
        return std::optional{AnimPaletteResolutionStrategy::palette_02};
    }
    if (lower_str == "pal-2") {
        return std::optional{AnimPaletteResolutionStrategy::palette_02};
    }
    if (lower_str == "pal_2") {
        return std::optional{AnimPaletteResolutionStrategy::palette_02};
    }
    if (lower_str == "pal2") {
        return std::optional{AnimPaletteResolutionStrategy::palette_02};
    }
    if (lower_str == "palette-2") {
        return std::optional{AnimPaletteResolutionStrategy::palette_02};
    }
    if (lower_str == "palette_2") {
        return std::optional{AnimPaletteResolutionStrategy::palette_02};
    }
    if (lower_str == "palette2") {
        return std::optional{AnimPaletteResolutionStrategy::palette_02};
    }
    // Fuzzy names for palette_03
    if (lower_str == "palette-03") {
        return std::optional{AnimPaletteResolutionStrategy::palette_03};
    }
    if (lower_str == "palette_03") {
        return std::optional{AnimPaletteResolutionStrategy::palette_03};
    }
    if (lower_str == "palette03") {
        return std::optional{AnimPaletteResolutionStrategy::palette_03};
    }
    if (lower_str == "pal-3") {
        return std::optional{AnimPaletteResolutionStrategy::palette_03};
    }
    if (lower_str == "pal_3") {
        return std::optional{AnimPaletteResolutionStrategy::palette_03};
    }
    if (lower_str == "pal3") {
        return std::optional{AnimPaletteResolutionStrategy::palette_03};
    }
    if (lower_str == "palette-3") {
        return std::optional{AnimPaletteResolutionStrategy::palette_03};
    }
    if (lower_str == "palette_3") {
        return std::optional{AnimPaletteResolutionStrategy::palette_03};
    }
    if (lower_str == "palette3") {
        return std::optional{AnimPaletteResolutionStrategy::palette_03};
    }
    // Fuzzy names for palette_04
    if (lower_str == "palette-04") {
        return std::optional{AnimPaletteResolutionStrategy::palette_04};
    }
    if (lower_str == "palette_04") {
        return std::optional{AnimPaletteResolutionStrategy::palette_04};
    }
    if (lower_str == "palette04") {
        return std::optional{AnimPaletteResolutionStrategy::palette_04};
    }
    if (lower_str == "pal-4") {
        return std::optional{AnimPaletteResolutionStrategy::palette_04};
    }
    if (lower_str == "pal_4") {
        return std::optional{AnimPaletteResolutionStrategy::palette_04};
    }
    if (lower_str == "pal4") {
        return std::optional{AnimPaletteResolutionStrategy::palette_04};
    }
    if (lower_str == "palette-4") {
        return std::optional{AnimPaletteResolutionStrategy::palette_04};
    }
    if (lower_str == "palette_4") {
        return std::optional{AnimPaletteResolutionStrategy::palette_04};
    }
    if (lower_str == "palette4") {
        return std::optional{AnimPaletteResolutionStrategy::palette_04};
    }
    // Fuzzy names for palette_05
    if (lower_str == "palette-05") {
        return std::optional{AnimPaletteResolutionStrategy::palette_05};
    }
    if (lower_str == "palette_05") {
        return std::optional{AnimPaletteResolutionStrategy::palette_05};
    }
    if (lower_str == "palette05") {
        return std::optional{AnimPaletteResolutionStrategy::palette_05};
    }
    if (lower_str == "pal-5") {
        return std::optional{AnimPaletteResolutionStrategy::palette_05};
    }
    if (lower_str == "pal_5") {
        return std::optional{AnimPaletteResolutionStrategy::palette_05};
    }
    if (lower_str == "pal5") {
        return std::optional{AnimPaletteResolutionStrategy::palette_05};
    }
    if (lower_str == "palette-5") {
        return std::optional{AnimPaletteResolutionStrategy::palette_05};
    }
    if (lower_str == "palette_5") {
        return std::optional{AnimPaletteResolutionStrategy::palette_05};
    }
    if (lower_str == "palette5") {
        return std::optional{AnimPaletteResolutionStrategy::palette_05};
    }
    // Fuzzy names for palette_06
    if (lower_str == "palette-06") {
        return std::optional{AnimPaletteResolutionStrategy::palette_06};
    }
    if (lower_str == "palette_06") {
        return std::optional{AnimPaletteResolutionStrategy::palette_06};
    }
    if (lower_str == "palette06") {
        return std::optional{AnimPaletteResolutionStrategy::palette_06};
    }
    if (lower_str == "pal-6") {
        return std::optional{AnimPaletteResolutionStrategy::palette_06};
    }
    if (lower_str == "pal_6") {
        return std::optional{AnimPaletteResolutionStrategy::palette_06};
    }
    if (lower_str == "pal6") {
        return std::optional{AnimPaletteResolutionStrategy::palette_06};
    }
    if (lower_str == "palette-6") {
        return std::optional{AnimPaletteResolutionStrategy::palette_06};
    }
    if (lower_str == "palette_6") {
        return std::optional{AnimPaletteResolutionStrategy::palette_06};
    }
    if (lower_str == "palette6") {
        return std::optional{AnimPaletteResolutionStrategy::palette_06};
    }
    // Fuzzy names for palette_07
    if (lower_str == "palette-07") {
        return std::optional{AnimPaletteResolutionStrategy::palette_07};
    }
    if (lower_str == "palette_07") {
        return std::optional{AnimPaletteResolutionStrategy::palette_07};
    }
    if (lower_str == "palette07") {
        return std::optional{AnimPaletteResolutionStrategy::palette_07};
    }
    if (lower_str == "pal-7") {
        return std::optional{AnimPaletteResolutionStrategy::palette_07};
    }
    if (lower_str == "pal_7") {
        return std::optional{AnimPaletteResolutionStrategy::palette_07};
    }
    if (lower_str == "pal7") {
        return std::optional{AnimPaletteResolutionStrategy::palette_07};
    }
    if (lower_str == "palette-7") {
        return std::optional{AnimPaletteResolutionStrategy::palette_07};
    }
    if (lower_str == "palette_7") {
        return std::optional{AnimPaletteResolutionStrategy::palette_07};
    }
    if (lower_str == "palette7") {
        return std::optional{AnimPaletteResolutionStrategy::palette_07};
    }
    // Fuzzy names for palette_08
    if (lower_str == "palette-08") {
        return std::optional{AnimPaletteResolutionStrategy::palette_08};
    }
    if (lower_str == "palette_08") {
        return std::optional{AnimPaletteResolutionStrategy::palette_08};
    }
    if (lower_str == "palette08") {
        return std::optional{AnimPaletteResolutionStrategy::palette_08};
    }
    if (lower_str == "pal-8") {
        return std::optional{AnimPaletteResolutionStrategy::palette_08};
    }
    if (lower_str == "pal_8") {
        return std::optional{AnimPaletteResolutionStrategy::palette_08};
    }
    if (lower_str == "pal8") {
        return std::optional{AnimPaletteResolutionStrategy::palette_08};
    }
    if (lower_str == "palette-8") {
        return std::optional{AnimPaletteResolutionStrategy::palette_08};
    }
    if (lower_str == "palette_8") {
        return std::optional{AnimPaletteResolutionStrategy::palette_08};
    }
    if (lower_str == "palette8") {
        return std::optional{AnimPaletteResolutionStrategy::palette_08};
    }
    // Fuzzy names for palette_09
    if (lower_str == "palette-09") {
        return std::optional{AnimPaletteResolutionStrategy::palette_09};
    }
    if (lower_str == "palette_09") {
        return std::optional{AnimPaletteResolutionStrategy::palette_09};
    }
    if (lower_str == "palette09") {
        return std::optional{AnimPaletteResolutionStrategy::palette_09};
    }
    if (lower_str == "pal-9") {
        return std::optional{AnimPaletteResolutionStrategy::palette_09};
    }
    if (lower_str == "pal_9") {
        return std::optional{AnimPaletteResolutionStrategy::palette_09};
    }
    if (lower_str == "pal9") {
        return std::optional{AnimPaletteResolutionStrategy::palette_09};
    }
    if (lower_str == "palette-9") {
        return std::optional{AnimPaletteResolutionStrategy::palette_09};
    }
    if (lower_str == "palette_9") {
        return std::optional{AnimPaletteResolutionStrategy::palette_09};
    }
    if (lower_str == "palette9") {
        return std::optional{AnimPaletteResolutionStrategy::palette_09};
    }
    // Fuzzy names for palette_10
    if (lower_str == "palette-10") {
        return std::optional{AnimPaletteResolutionStrategy::palette_10};
    }
    if (lower_str == "palette_10") {
        return std::optional{AnimPaletteResolutionStrategy::palette_10};
    }
    if (lower_str == "palette10") {
        return std::optional{AnimPaletteResolutionStrategy::palette_10};
    }
    if (lower_str == "pal-10") {
        return std::optional{AnimPaletteResolutionStrategy::palette_10};
    }
    if (lower_str == "pal_10") {
        return std::optional{AnimPaletteResolutionStrategy::palette_10};
    }
    if (lower_str == "pal10") {
        return std::optional{AnimPaletteResolutionStrategy::palette_10};
    }
    // Fuzzy names for palette_11
    if (lower_str == "palette-11") {
        return std::optional{AnimPaletteResolutionStrategy::palette_11};
    }
    if (lower_str == "palette_11") {
        return std::optional{AnimPaletteResolutionStrategy::palette_11};
    }
    if (lower_str == "palette11") {
        return std::optional{AnimPaletteResolutionStrategy::palette_11};
    }
    if (lower_str == "pal-11") {
        return std::optional{AnimPaletteResolutionStrategy::palette_11};
    }
    if (lower_str == "pal_11") {
        return std::optional{AnimPaletteResolutionStrategy::palette_11};
    }
    if (lower_str == "pal11") {
        return std::optional{AnimPaletteResolutionStrategy::palette_11};
    }
    // Fuzzy names for palette_12
    if (lower_str == "palette-12") {
        return std::optional{AnimPaletteResolutionStrategy::palette_12};
    }
    if (lower_str == "palette_12") {
        return std::optional{AnimPaletteResolutionStrategy::palette_12};
    }
    if (lower_str == "palette12") {
        return std::optional{AnimPaletteResolutionStrategy::palette_12};
    }
    if (lower_str == "pal-12") {
        return std::optional{AnimPaletteResolutionStrategy::palette_12};
    }
    if (lower_str == "pal_12") {
        return std::optional{AnimPaletteResolutionStrategy::palette_12};
    }
    if (lower_str == "pal12") {
        return std::optional{AnimPaletteResolutionStrategy::palette_12};
    }
    // Fuzzy names for palette_13
    if (lower_str == "palette-13") {
        return std::optional{AnimPaletteResolutionStrategy::palette_13};
    }
    if (lower_str == "palette_13") {
        return std::optional{AnimPaletteResolutionStrategy::palette_13};
    }
    if (lower_str == "palette13") {
        return std::optional{AnimPaletteResolutionStrategy::palette_13};
    }
    if (lower_str == "pal-13") {
        return std::optional{AnimPaletteResolutionStrategy::palette_13};
    }
    if (lower_str == "pal_13") {
        return std::optional{AnimPaletteResolutionStrategy::palette_13};
    }
    if (lower_str == "pal13") {
        return std::optional{AnimPaletteResolutionStrategy::palette_13};
    }
    // Fuzzy names for palette_14
    if (lower_str == "palette-14") {
        return std::optional{AnimPaletteResolutionStrategy::palette_14};
    }
    if (lower_str == "palette_14") {
        return std::optional{AnimPaletteResolutionStrategy::palette_14};
    }
    if (lower_str == "palette14") {
        return std::optional{AnimPaletteResolutionStrategy::palette_14};
    }
    if (lower_str == "pal-14") {
        return std::optional{AnimPaletteResolutionStrategy::palette_14};
    }
    if (lower_str == "pal_14") {
        return std::optional{AnimPaletteResolutionStrategy::palette_14};
    }
    if (lower_str == "pal14") {
        return std::optional{AnimPaletteResolutionStrategy::palette_14};
    }
    // Fuzzy names for palette_15
    if (lower_str == "palette-15") {
        return std::optional{AnimPaletteResolutionStrategy::palette_15};
    }
    if (lower_str == "palette_15") {
        return std::optional{AnimPaletteResolutionStrategy::palette_15};
    }
    if (lower_str == "palette15") {
        return std::optional{AnimPaletteResolutionStrategy::palette_15};
    }
    if (lower_str == "pal-15") {
        return std::optional{AnimPaletteResolutionStrategy::palette_15};
    }
    if (lower_str == "pal_15") {
        return std::optional{AnimPaletteResolutionStrategy::palette_15};
    }
    if (lower_str == "pal15") {
        return std::optional{AnimPaletteResolutionStrategy::palette_15};
    }
    // Fuzzy names for internal_png_palette
    if (lower_str == "internal-png-palette") {
        return std::optional{AnimPaletteResolutionStrategy::internal_png_palette};
    }
    if (lower_str == "internal_png_palette") {
        return std::optional{AnimPaletteResolutionStrategy::internal_png_palette};
    }
    if (lower_str == "internal-png-pal") {
        return std::optional{AnimPaletteResolutionStrategy::internal_png_palette};
    }
    if (lower_str == "internal_png_pal") {
        return std::optional{AnimPaletteResolutionStrategy::internal_png_palette};
    }
    if (lower_str == "internalpngpal") {
        return std::optional{AnimPaletteResolutionStrategy::internal_png_palette};
    }
    // Fuzzy names for scan_all_tilesets
    if (lower_str == "scan-all-tilesets") {
        return std::optional{AnimPaletteResolutionStrategy::scan_all_tilesets};
    }
    if (lower_str == "scan_all_tilesets") {
        return std::optional{AnimPaletteResolutionStrategy::scan_all_tilesets};
    }
    if (lower_str == "scanalltilesets") {
        return std::optional{AnimPaletteResolutionStrategy::scan_all_tilesets};
    }
    if (lower_str == "full-tileset-scan") {
        return std::optional{AnimPaletteResolutionStrategy::scan_all_tilesets};
    }
    if (lower_str == "full_tileset_scan") {
        return std::optional{AnimPaletteResolutionStrategy::scan_all_tilesets};
    }
    if (lower_str == "fulltilesetscan") {
        return std::optional{AnimPaletteResolutionStrategy::scan_all_tilesets};
    }

    return std::nullopt;
}

/// @brief Converts a AnimPaletteResolutionStrategy to its canonical string representation.
///
/// @details
/// Returns the C++ constant name (snake_case). This is the canonical form used for
/// general purpose string conversion, logging, and error messages.
///
/// @param m The value to convert
/// @return The canonical snake_case string representation
[[nodiscard]] inline std::string to_string(const AnimPaletteResolutionStrategy m)
{
    switch (m) {
    case AnimPaletteResolutionStrategy::scan_local_metatiles:
        return "scan_local_metatiles";
    case AnimPaletteResolutionStrategy::palette_00:
        return "palette_00";
    case AnimPaletteResolutionStrategy::palette_01:
        return "palette_01";
    case AnimPaletteResolutionStrategy::palette_02:
        return "palette_02";
    case AnimPaletteResolutionStrategy::palette_03:
        return "palette_03";
    case AnimPaletteResolutionStrategy::palette_04:
        return "palette_04";
    case AnimPaletteResolutionStrategy::palette_05:
        return "palette_05";
    case AnimPaletteResolutionStrategy::palette_06:
        return "palette_06";
    case AnimPaletteResolutionStrategy::palette_07:
        return "palette_07";
    case AnimPaletteResolutionStrategy::palette_08:
        return "palette_08";
    case AnimPaletteResolutionStrategy::palette_09:
        return "palette_09";
    case AnimPaletteResolutionStrategy::palette_10:
        return "palette_10";
    case AnimPaletteResolutionStrategy::palette_11:
        return "palette_11";
    case AnimPaletteResolutionStrategy::palette_12:
        return "palette_12";
    case AnimPaletteResolutionStrategy::palette_13:
        return "palette_13";
    case AnimPaletteResolutionStrategy::palette_14:
        return "palette_14";
    case AnimPaletteResolutionStrategy::palette_15:
        return "palette_15";
    case AnimPaletteResolutionStrategy::internal_png_palette:
        return "internal_png_palette";
    case AnimPaletteResolutionStrategy::scan_all_tilesets:
        return "scan_all_tilesets";
    }
    panic("unhandled AnimPaletteResolutionStrategy value");
}

/// @brief Stream insertion operator for AnimPaletteResolutionStrategy.
///
/// @param os The output stream
/// @param m The value to output
/// @return Reference to the output stream
inline std::ostream &operator<<(std::ostream &os, const AnimPaletteResolutionStrategy m)
{
    return os << to_string(m);
}

} // namespace porytiles

template <>
struct std::formatter<porytiles::AnimPaletteResolutionStrategy> {
    constexpr auto parse(std::format_parse_context &ctx)
    {
        return ctx.begin();
    }

    auto format(const porytiles::AnimPaletteResolutionStrategy &value, auto &ctx) const
    {
        return std::format_to(ctx.out(), "{}", porytiles::to_string(value));
    }
};