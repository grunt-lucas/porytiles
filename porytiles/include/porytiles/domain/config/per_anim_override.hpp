...
///
/// 1. **Per-tile** (@c per_tile_palette_resolution_strategies): most specific. When non-empty, its length must exactly
///    match the animation's @c tile_count. Each entry can be empty (no value) to fall through to the next tier.
/// 2. **Per-anim** (@c palette_resolution_strategy): middle tier. A single strategy applied to all subtiles of this
...
/// @c multi_palette_subtile_resolution_strategy wins, otherwise falls back to
/// @c multi_palette_subtile_resolution_strategy from DomainConfig.
///
