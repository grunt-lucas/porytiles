...
///
/// @details
/// ShapeMask uses 1 bit per pixel to track the "shape" of 8x8 tile. Each of the 8 rows is stored as a uint8_t, where
...
/// to a specific color index. The flipping operations and lexicographic comparison enable the compiler to find
/// canonical (minimal) orientations for tiles in a color-agnostic way, allowing deduplication of tiles that are
/// identical under flips or color transformations.
///
...
template <>
struct std::hash<porytiles::ShapeMask> {
