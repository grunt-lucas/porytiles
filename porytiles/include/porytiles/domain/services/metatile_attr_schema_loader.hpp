...
/// validated against @c attr_bytes. @c resolved_specs is the full post-merge spec list (both layouts' masks), useful
/// for diagnostics and for reporting which fields were excluded for the chosen layout. @c attr_bytes is the byte width
/// the schema was validated against: for the primary layout it may have been widened past the detected size to cover
/// the selected masks, and for the frlg layout it is always 4 (the engine reads FRLG-layout attributes through a
/// hardcoded 'const u32 *' cast). @c declaration_bytes is the width for generated @c gMetatileAttributes_* C
/// declarations: the authoritative declared width when one exists, else @c attr_bytes. It may be narrower than @c
/// attr_bytes for frlg, matching pokeemerald-expansion, whose FRLG tilesets are declared 'const u16' but read as 4-byte
/// words.
