 ...
 * either (defines first, then enums). During the scan, names that lack the spec's prefix, names in the spec's skip set,
 * and values that exceed the field's maximum are silently ignored, so an unrelated or over-wide constant in a shared
 * header is simply not a mapping. The provider loads lazily on first lookup and caches the result.
 */
class HeaderEnumMapProvider final : public EnumMapProvider {
  ...
  private:
     ...
     * out (returning success without inserting). Otherwise it checks for duplicate names and values, and on a duplicate
     * produces a rich error showing both source locations.
     *
