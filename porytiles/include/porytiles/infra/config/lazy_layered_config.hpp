...
class LazyLayeredConfig final : public DomainConfig, public AppConfig, public InfraConfig {
  ...
  protected:
    ...
    packing_strategy_params_raw(ConfigScopeType type, const std::string &scope) const override;

    [[nodiscard]] ChainableResult<ConfigValue<TileSharingPacking>>
    tile_sharing_packing_raw(ConfigScopeType type, const std::string &scope) const override;
    ...
    [[nodiscard]] ChainableResult<ConfigValue<bool>>
    create_sample_anims_raw(ConfigScopeType type, const std::string &scope) const override;
  ...
  public:
    ...
    /// @param scope The scope identifier
    /// @return Vector of ProvenanceChainLink entries, one per provider
    [[nodiscard]] std::vector<ProvenanceChainLink<PackingStrategyType>>
    packing_strategy_provenance_chain(ConfigScopeType type, const std::string &scope) const;

    /// @brief Gets the full provenance chain for packing_strategy_params.
    ///
    /// @details
    /// Returns what each provider in the chain would return for this config value, from highest priority to lowest.
    /// Unlike the normal resolution which stops at the first valid/invalid result, this queries ALL providers to give
    ...
    /// @param scope The scope identifier
    /// @return Vector of ProvenanceChainLink entries, one per provider
    [[nodiscard]] std::vector<ProvenanceChainLink<PackingStrategyParams>>
    packing_strategy_params_provenance_chain(ConfigScopeType type, const std::string &scope) const;

    /// @brief Gets the full provenance chain for tile_sharing_packing.
    ///
    /// @details
    /// Returns what each provider in the chain would return for this config value, from highest priority to lowest.
    /// Unlike the normal resolution which stops at the first valid/invalid result, this queries ALL providers to give
    ...
    /// @param scope The scope identifier
    /// @return Vector of ProvenanceChainLink entries, one per provider
    [[nodiscard]] std::vector<ProvenanceChainLink<TileSharingPacking>>
    tile_sharing_packing_provenance_chain(ConfigScopeType type, const std::string &scope) const;

    /// @brief Gets the full provenance chain for tile_sharing_alignment.
    ///
    /// @details
    /// Returns what each provider in the chain would return for this config value, from highest priority to lowest.
    /// Unlike the normal resolution which stops at the first valid/invalid result, this queries ALL providers to give
    ...
    /// @brief Gets the full provenance chain for create_sample_anims.
    ///
    /// @details
    /// Returns what each provider in the chain would return for this config value, from highest priority to lowest.
    /// Unlike the normal resolution which stops at the first valid/invalid result, this queries ALL providers to give
    ...
    /// @return Vector of ProvenanceChainLink entries, one per provider
    [[nodiscard]] std::vector<ProvenanceChainLink<bool>>
    create_sample_anims_provenance_chain(ConfigScopeType type, const std::string &scope) const;
    ...
    /// @brief Gets the full provenance chain for metatile_attribute_size.
    ///
    /// @details
    /// Returns what each provider in the chain would return for this config value, from highest priority to lowest.
    /// Unlike the normal resolution which stops at the first valid/invalid result, this queries ALL providers to give
    ...
    /// @param scope The scope identifier
    /// @return Vector of ProvenanceChainLink entries, one per provider
    [[nodiscard]] std::vector<ProvenanceChainLink<std::optional<std::size_t>>>
    metatile_attribute_size_provenance_chain(ConfigScopeType type, const std::string &scope) const;

    /// @brief Gets the full provenance chain for metatile_attribute_declaration_size.
    ///
    /// @details
    /// Returns what each provider in the chain would return for this config value, from highest priority to lowest.
    /// Unlike the normal resolution which stops at the first valid/invalid result, this queries ALL providers to give
    ...
    /// @return Vector of ProvenanceChainLink entries, one per provider
    [[nodiscard]] std::vector<ProvenanceChainLink<std::optional<std::size_t>>>
    metatile_attribute_declaration_size_provenance_chain(ConfigScopeType type, const std::string &scope) const;
