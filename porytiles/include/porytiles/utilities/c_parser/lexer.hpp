...
class Lexer {
  public:
    ...
    ///
    /// @details
    /// This constructor creates a lexer without a CParserContext. Errors will be formatted as simple "line:col:
    ...
    /// token. If an error is encountered (e.g., unterminated string), returns a ChainableResult containing an error
    /// with source position information.
    ///
    /// When constructed with a CParserContext, errors include source code context with highlighted error locations.
    /// When constructed without a context, errors are simple "line:col: message" format.
    ///
