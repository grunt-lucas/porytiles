...
/// member name such as `METATILE_ATTRIBUTE_BEHAVIOR`, or a numeric literal). @c value holds the evaluated value when
/// the value expression could be resolved from the parser's symbol table; it is absent when the expression referenced
/// an unknown symbol. @c value_tokens keeps the raw value expression so a caller can re-evaluate it against a richer
/// symbol table later.
