...
///
/// @details
/// Tolerant scans do not abort when a single construct fails to evaluate (for example a #define whose value references
/// a macro from an unparsed header). Instead the offender is captured here with enough context to report it, and
/// scanning continues. The name still counts as defined for later conditional decisions.
