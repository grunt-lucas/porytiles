...
/// conditions to users. The class supports multiple severity levels and integrates with the ChainableResult error
/// system to provide hierarchical error chain visualization.
///
...
class UserDiagnostics {
  public:
    ...
    ///
    /// @details
    /// Enables inline formatting of error note messages by accepting a format string and variadic FormatParam
    /// arguments. The format string uses fmtlib-style `{}` placeholders.
    ///
    /// Example:
    /// ```C++
