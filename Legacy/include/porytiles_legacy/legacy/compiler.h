...
using ColorSet = std::pair<std::bitset<porytiles_legacy::MAX_BG_PALETTES * (porytiles_legacy::PAL_SIZE - 1)>, std::size_t>;

template <> struct std::hash<ColorSet> {
  std::size_t operator()(const ColorSet &colorSet) const noexcept
  {
    // TODO : better hash function
    return std::hash<std::bitset<porytiles_legacy::MAX_BG_PALETTES * (porytiles_legacy::PAL_SIZE - 1)>>{}(colorSet.first) ^
...
using IndexAndNormTile = std::pair<porytiles_legacy::DecompiledIndex, porytiles_legacy::NormalizedTile>;
using IndexedNormTileWithColorSet = std::tuple<porytiles_legacy::DecompiledIndex, porytiles_legacy::NormalizedTile, ColorSet>;
