...
template <> struct std::hash<porytiles_legacy::BGR15> {
    std::size_t operator()(const porytiles_legacy::BGR15 &bgr) const noexcept {
...
template <> struct std::hash<porytiles_legacy::RGBA32> {
    std::size_t operator()(const porytiles_legacy::RGBA32 &rgba) const noexcept {
...
template <> struct std::hash<porytiles_legacy::GBATile> {
    std::size_t operator()(const porytiles_legacy::GBATile &tile) const noexcept {
        // TODO : better hash function
...
template <> struct std::hash<porytiles_legacy::NormalizedPixels> {
    std::size_t operator()(const porytiles_legacy::NormalizedPixels &pixels) const noexcept {
        // TODO : better hash function
        std::size_t hashValue = 0;
...
template <> struct std::hash<porytiles_legacy::NormalizedPalette> {
    std::size_t operator()(const porytiles_legacy::NormalizedPalette &palette) const noexcept {
        // TODO : better hash function
        std::size_t hashValue = 0;
        hashValue ^= std::hash<int>{}(palette.size);
        for (auto color : palette.colors) {
            hashValue ^= std::hash<porytiles_legacy::BGR15>{}(color);
        }
        return hashValue;
    }
};
...
struct NormalizedTile {
    ...
    void setPixel(const std::size_t frame, const std::size_t row, const std::size_t col, const uint8_t value) {
        ...
        if (row >= TILE_SIDE_LENGTH_PIX) {
            throw std::out_of_range{"internal: NormalizedTile::setPixel row argument out of bounds (" +
                                    std::to_string(row) + ")"};
        }
        if (col >= TILE_SIDE_LENGTH_PIX) {
            throw std::out_of_range{"internal: NormalizedTile::setPixel col argument out of bounds (" +
                                    std::to_string(col) + ")"};
        }
...
template <> struct std::hash<porytiles_legacy::NormalizedTile> {
    std::size_t operator()(const porytiles_legacy::NormalizedTile &tile) const noexcept {
        // TODO : better hash function
        std::size_t hashValue = 0;
        for (const auto &layer : tile.frames) {
            hashValue ^= std::hash<porytiles_legacy::NormalizedPixels>{}(layer);
        }
        hashValue ^= std::hash<porytiles_legacy::NormalizedPalette>{}(tile.palette);
