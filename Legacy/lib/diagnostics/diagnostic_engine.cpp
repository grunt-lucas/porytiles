...
std::optional<std::string> construct_flag(const porytiles_legacy::DiagLevel in_flight_level,
                                          const porytiles_legacy::DiagTempl &templ) {
    if (in_flight_level == porytiles_legacy::DiagLevel::Warning) {
        return std::optional{fmt::format("-W{}", templ.name())};
    }
    if (templ.level() == porytiles_legacy::DiagLevel::Warning && in_flight_level == porytiles_legacy::DiagLevel::Error) {
