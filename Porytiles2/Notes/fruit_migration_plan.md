# Google Fruit DI Migration Plan for Porytiles2

**Document Version:** 1.0
**Created:** 2025-10-21
**Target Codebase:** Porytiles2
...
**File:** `CMakeLists.txt` (root or Porytiles2/CMakeLists.txt)
...
target_link_libraries(porytiles2 PRIVATE fruit)
```
...
target_link_libraries(Porytiles2Lib PUBLIC Fruit::fruit)
target_link_libraries(porytiles2 PRIVATE Fruit::fruit)
```
...
**File:** `Porytiles2/include/porytiles2/di/cli_config.hpp`
...
**File:** `Porytiles2/include/porytiles2/di/components.hpp`
...
#include "porytiles2/di/cli_config.hpp"
...
**File:** `Porytiles2/lib/di/components.cpp`
...
cmake -B build -DCMAKE_BUILD_TYPE=Debug
cmake --build build -j7
./build/Porytiles2/tests/Porytiles2AllTests
```

### Step 3.2: Create DI Infrastructure (Parallel to Existing Code)

1. Create `Porytiles2/include/porytiles2/di/` directory
2. Add `cli_config.hpp`, `components.hpp`
3. Create `Porytiles2/lib/di/` directory
...
**File:** `Porytiles2/tools/driver/debug_commands.hpp`
...
3. **Manual functional test:**
   ```bash
   # Test with color (TTY)
   ./build/Porytiles2/tools/driver/porytiles2 debug-compile-primary test_tileset

   # Test without color (flag)
   ./build/Porytiles2/tools/driver/porytiles2 debug-compile-primary test_tileset --no-color

   # Test without color (redirect)
   ./build/Porytiles2/tools/driver/porytiles2 debug-compile-primary test_tileset 2> /tmp/out.txt
...
**File:** `Porytiles2/include/porytiles2/di/components.hpp` (addition)
...
**File:** `Porytiles2/lib/di/components.cpp` (addition)
...
**Before:**
```C++
// Porytiles2/tools/driver/debug_commands.hpp (old)
class DebugPrimaryCompileCommand final : public Command {
public:
    explicit DebugPrimaryCompileCommand(CLI::App &parent_app)
        : Command{parent_app, kCommandName, kCommandDesc, kCommandGroup}
...
**After:**
```C++
// Porytiles2/tools/driver/debug_commands.hpp (new)
#include "porytiles2/di/components.hpp"
...
**File:** `Porytiles2/tests/integration/di_integration_test.cpp`
...
./build/Porytiles2/tools/driver/porytiles2 debug-compile-primary test_tileset 2>&1 | grep -q "\033\[" && echo "PASS: ANSI codes present" || echo "FAIL: No ANSI codes"

# Test 2: No color with flag
echo "Test 2: No color with --no-color flag"
./build/Porytiles2/tools/driver/porytiles2 debug-compile-primary test_tileset --no-color 2>&1 | grep -q "\033\[" && echo "FAIL: ANSI codes present" || echo "PASS: No ANSI codes"

# Test 3: No color when redirected
echo "Test 3: No color when redirected to file"
./build/Porytiles2/tools/driver/porytiles2 debug-compile-primary test_tileset 2> /tmp/porytiles_output.txt
...
**Error:**
```
error: No constructor was specified for type porytiles2::PrimaryTilesetCompiler, ...
```
