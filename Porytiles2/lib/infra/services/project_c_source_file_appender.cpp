...
porytiles2::Result<void> write_file(const std::filesystem::path &file_path, const std::string &content) {
    try {
        ...
        return std::unexpected{fmt::format("Exception writing file {}: {}", file_path.string(), e.what())};
    }
}
porytiles2::Result<void> append_to_file(const std::filesystem::path &file_path, const std::string &content) {
    // Read existing file content
    auto existing_content_result = read_file(file_path);
    if (!existing_content_result) {
        return std::unexpected{
            fmt::format("Failed to read file {}: {}", file_path.string(), existing_content_result.error())};
    }
...
Result<void> ProjectCSourceFileAppender::append_tileset_declarations(const std::string &tileset_name) {
