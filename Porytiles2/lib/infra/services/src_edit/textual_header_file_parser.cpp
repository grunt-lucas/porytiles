...
Result<std::vector<std::string>> read_file_lines(const std::filesystem::path &file_path) {
    try {
        ...
        return std::unexpected{fmt::format("Exception reading file {}: {}", file_path.string(), e.what())};
    }
}
...
Result<std::vector<std::string>> TextualHeaderFileParser::parse_header_file(const std::filesystem::path &file_path) {
...
Result<bool> TextualHeaderFileParser::contains_declaration(const std::filesystem::path &file_path,
    ...
    if (!lines_result) {
        return std::unexpected{fmt::format("Failed to read file {}: {}", file_path.string(), lines_result.error())};
    }
...
Result<size_t> TextualHeaderFileParser::find_insertion_point(const std::filesystem::path &file_path) {
    auto lines_result = read_file_lines(file_path);
    if (!lines_result) {
        return std::unexpected{fmt::format("Failed to read file {}: {}", file_path.string(), lines_result.error())};
    }
...
Result<bool> TextualHeaderFileParser::validate_header_structure(const std::filesystem::path &file_path) {
    ...
    if (!has_c_content) {
        return std::unexpected{fmt::format("File does not appear to contain C header content: {}", file_path.string())};
    }
