...
TEST(CImgPngTests, OpenShouldFailGracefullyOnBadFile) {
    ...
    EXPECT_TRUE(result.error().contains("Failed to open file 'Resources/Tests/png/non_existent_file.png'"));

    const auto result2 = png.Read("Resources/Tests/metatile_behaviors.h");
    ASSERT_FALSE(result2.has_value());
    EXPECT_TRUE(result2.error().contains("Failed to recognize format of file 'Resources/Tests/metatile_behaviors.h'"));
}
