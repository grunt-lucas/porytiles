...
TEST(WarningTests, WallShouldEnableAllWarnings) {
    ...
    ASSERT_EQ(engine.EnabledAt(WarnColorPrecisionLoss), DiagLevel::Ignored);
    ASSERT_EQ(engine.EnabledAt(WarnTransparencyCollapse), DiagLevel::Ignored);
    ASSERT_EQ(engine.EnabledAt(WarnUnusedAttribute), DiagLevel::Ignored);
