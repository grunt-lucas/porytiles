...
protected:
  void SetUp() override {
    ...
    appender_ = std::make_unique<ProjectCSourceFileAppender>(gsl::not_null(project_paths_.get()),
                                                             std::move(generator));
  }
...
const u16 gMetatileAttributes_General[] = INCBIN_U16("data/tilesets/primary/general/metatile_attributes.bin");
  ...
  void assert_file_does_not_contain(const std::filesystem::path &file_path,
...
TEST_F(ProjectCSourceFileAppenderIntegrationTest,
  ...
  assert_file_contains(project_paths_->headers_header(),
                       ".palettes = gTilesetPalettes_MyNewTileset");
}
...
TEST_F(ProjectCSourceFileAppenderIntegrationTest,
  ...
  assert_file_contains(project_paths_->metatiles_header(),
                       "my_new_tileset/metatile_attributes.bin");
}
...
TEST_F(ProjectCSourceFileAppenderIntegrationTest, MultipleAppendsShouldNotCorruptFiles) {
  ...
  for (const auto &tileset : tilesets) {
    assert_file_contains(project_paths_->graphics_header(),
                         fmt::format("gTilesetPalettes_{}", tileset));
    assert_file_contains(project_paths_->graphics_header(),
                         fmt::format("gTilesetTiles_{}", tileset));
