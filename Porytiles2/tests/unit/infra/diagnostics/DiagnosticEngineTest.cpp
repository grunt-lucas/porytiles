...
TEST(WarningTests, WallShouldEnableAllWarnings) {
  ...
  EXPECT_EQ(engine.enabled_at(warn_transparency_collapse), DiagLevel::ignored);
  EXPECT_EQ(engine.enabled_at(warn_unused_attribute), DiagLevel::ignored);

  // RGBATile tile{};
  // engine.Report(kWarnColorPrecisionLoss, tile, std::string{"foo"},
  // std::string{"bar"}, 0UL, 0UL); engine.Report(kWarnColorPrecisionLoss, tile,
  // std::string{"foo"}, std::string{"bar"}, 0UL, 0UL);
  // engine.Report(kWarnColorPrecisionLoss, tile, std::string{"foo"},
  // std::string{"bar"}, 0UL, 0UL);
  // engine.ReportPartner(kWarnColorPrecisionLoss, 0, tile, std::string{"foo"},
  // 0UL, 0UL); ASSERT_EQ(engine.consumer().ConsumedCount(), 0);
  ...
  // std::string{"bar"}, 0UL, 0UL);
  // engine.ReportPartner(kWarnColorPrecisionLoss, 0, tile, std::string{"foo"},
  // 0UL, 0UL); ASSERT_EQ(engine.consumer().ConsumedCount(), 2);
...
TEST(WarningTests, IndividualWarningsShouldExplicitlyEnable) {
  ...
  // RGBATile tile{};
  // engine.Report(kWarnColorPrecisionLoss, tile, std::string{"foo"},
  // std::string{"bar"}, 0UL, 0UL); ASSERT_EQ(engine.consumer().ConsumedCount(),
  // 0); ASSERT_EQ(engine.InFlightCountForLevel(DiagLevel::Warning), 0);
  ...
  // engine.Report(kWarnColorPrecisionLoss, tile, std::string{"foo"},
  // std::string{"bar"}, 0UL, 0UL); ASSERT_EQ(engine.consumer().ConsumedCount(),
  // 1); ASSERT_EQ(engine.InFlightCountForLevel(DiagLevel::Warning), 1);

  // engine.EnableAtLevel(kWarnColorPrecisionLoss, DiagLevel::Error);
  // engine.Report(kWarnColorPrecisionLoss, tile, std::string{"foo"},
  // std::string{"bar"}, 0UL, 0UL); ASSERT_EQ(engine.consumer().ConsumedCount(),
  // 2); ASSERT_EQ(engine.InFlightCountForLevel(DiagLevel::Warning), 1);
  // ASSERT_EQ(engine.InFlightCountForLevel(DiagLevel::Error), 1);

  // // Disabling at warning doesn't change anything, since it's still enabled
  // at error level engine.DisableAtLevel(kWarnColorPrecisionLoss,
  // DiagLevel::Warning); engine.Report(kWarnColorPrecisionLoss, tile,
  // std::string{"foo"}, std::string{"bar"}, 0UL, 0UL);
  // ASSERT_EQ(engine.consumer().ConsumedCount(), 3);
  // ASSERT_EQ(engine.InFlightCountForLevel(DiagLevel::Warning), 1);
  ...
  // engine.Report(kWarnColorPrecisionLoss, tile, std::string{"foo"},
  // std::string{"bar"}, 0UL, 0UL); ASSERT_EQ(engine.consumer().ConsumedCount(),
  // 3); ASSERT_EQ(engine.InFlightCountForLevel(DiagLevel::Warning), 1);
  // ASSERT_EQ(engine.InFlightCountForLevel(DiagLevel::Error), 2);
}
