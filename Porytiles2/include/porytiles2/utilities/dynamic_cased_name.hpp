...
template <>
struct std::hash<porytiles2::DynamicCasedName> {
    std::size_t operator()(const porytiles2::DynamicCasedName &name) const noexcept
    {
 ...
 * porytiles2::to_string() overload for consistent string representation.
 */
template <>
struct std::formatter<porytiles2::DynamicCasedName> {
    constexpr auto parse(std::format_parse_context &ctx)
    {
        return ctx.begin();
    }

    auto format(const porytiles2::DynamicCasedName &value, auto &ctx) const
    {
