...
class TextualHeaderFileParser final : public HeaderFileParser {
  public:
     ...
     */
    [[nodiscard]] Result<std::vector<std::string>> parse_header_file(const std::filesystem::path &file_path) override;
     ...
     */
    [[nodiscard]] Result<bool> validate_header_structure(const std::filesystem::path &file_path) override;
};
