 ...
 *
 * @details
 * Constructs final Rgba32 palettes from PackedPalette color sets using a four-phase algorithm. The multi-phase
 * approach — sequential fill skipping Indirect colors, then resolving Indirect chains to Absolute positions — is
 * inspired by the Indirection resolution in the @c assign_palettes function from borytiles by ishax-kos
 * (https://github.com/ishax-kos/borytiles), specifically its @c compilation.rs module. Porytiles2 separates the
 * phases more explicitly and adds eviction logic for slot conflicts with non-prefilled colors.
 *
 ...
 * prevents cycles).
 *
 * **Phase 3 — Sequential fill**: For each palette, collect slots already used by Absolute positions. Assign each
 * remaining Undetermined color to the next free slot as Absolute. Indirect colors are skipped — they don't compete
 * for slots.
 *
 * **Phase 4 — Resolve Indirect chains**: For each Indirect color, follow the chain ref_pal[ref_color] until hitting
 * an Absolute position. Cap at @c pal::num_pals iterations for cycle detection (panic on cycle). Place the color at
 * the resolved slot. If the slot conflicts with a prefilled position, skip (best-effort).
 *
 ...
 * @param indirect_links The Indirect link instructions (empty for manual mode).
 * @param failure_counts Optional output pointer for alignment failure counters. When non-null, incremented at each
 *     Phase 2 and Phase 4 skip point. Caller retains ownership.
